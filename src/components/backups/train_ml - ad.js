...
export default class Test extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      bin: '15',
      shuffledata: false,
      status: props.value.data.mstatus
    };
  ...
  handleShuffledataChange = (event) => {
    console.log("shuffledata console", event.target.checked)
    this.setState({
  ...
  handleUploadImage(ev) {
    ...
    setInterval(() => {
      ...
      console.log("interval props", this.props)
      fetch("/result_update")
      .then((res) => res.json()
        .then((statusdata) => {
           ...
           this.props.value.setData({
             trained_r: statusdata.training_r_squared_value,
             tested_r: statusdata.testing_r_squared_value,
             mstatus: statusdata.mstatus,
             flag: true
          })
        })
      )
