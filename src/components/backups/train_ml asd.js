...
export default class Test extends React.Component {
  ...
  handleUploadImage(ev) {
    ...
    //if (this.status != "Done!") {
    //console.log("about to run status fetch")
    //setInterval(() => {
    ...
    //console.log("Before timeinterval = ", this.state.status)
    //var counter = 0
    //const myInterval = setInterval(myTimer, 10000);
    //
    //function myTimer() {
    //
    //  console.log("interval happening counter = ", counter)
    //  fetch("/training_status").then((res) =>
    //    res.json().then((statusdata) => {
    //      console.log("bringing data from backend = ", statusdata)
    //      // Setting a data from api
    ...
    //    myStopFunction()
    //  }
    //  console.log("after timeinterval = ", this.state)
    //}
    ...
    var i = "in progress.."
    while (i !== "Done!" ) {
      console.log("here is thevalue for i", i)
      console.log(this.state.status)
      var api=fetch("/training_status").then((res) =>
        res.json().then((statusdata) => {
          console.log("bringing data from backend = ", statusdata)
          // Setting a data from api
          this.setState({ status: statusdata.mstatus })
      ...
      console.log("API = ", api)
      console.log(api.Promise)
      console.log(api.PromiseState)
      console.log(api.PromiseResult)
          ...
          <form className="uploader" onSubmit={this.handleUploadImage}>
            <div className="training_content">
              ...
              <div className="status">
                <p>status value =  {this.state.status}</p>
              </div>
